    ...
    <header className="mx-auto flex max-w-6xl items-center justify-between gap-4 py-5">
      <Link href="/" className="focus-ring flex items-center gap-2 rounded-sm" aria-label="DereType Quiz home">
        <Image
          ...
          height={52}
          aria-hidden="true"
          className="h-11 w-11 rounded-sm border-3 border-ink object-cover shadow-[3px_3px_0_#171313] md:h-13 md:w-13"
          priority
        />
        <span className="font-display text-xl font-black md:text-3xl">DereType Quiz</span>
      </Link>
      {!compact && (
        <nav className="hidden items-center gap-2 text-sm font-black lg:flex">
          <Link className="focus-ring manga-nav-tab bg-punch text-white" href="/quiz">Quiz</Link>
          <Link className="focus-ring manga-nav-tab bg-panel" href="/dere-types">Dere Types</Link>
          <Link className="focus-ring manga-nav-tab bg-panel" href="/tsundere-vs-yandere">Tsundere vs Yandere</Link>
          <Link className="focus-ring manga-nav-tab bg-panel" href="/are-you-a-yandere">Are You a Yandere?</Link>
        </nav>
      )}
      <div className="flex items-center gap-3">
        <Link
          href="/quiz"
          className="focus-ring hidden rounded-md border-3 border-ink bg-punch px-5 py-3 text-sm font-black text-white shadow-[4px_4px_0_#171313] transition-transform hover:-translate-y-0.5 lg:inline-flex"
        >
          Start Quiz
        </Link>
        {!compact && (
          <details className="relative lg:hidden">
            ...
            <nav className="absolute right-0 z-20 mt-3 grid w-60 gap-3 border-3 border-ink bg-panel p-4 text-sm font-black shadow-manga">
